...
class Dimensions(object):
    ...
    def __init__(self,
                 width=D.width,
                 height=D.height,
                 vertical_margin=D.vertical_margin,
                 horizontal_margin=D.horizontal_margin,
                 goalpost_length=D.goalpost_length,
                 arc_radius=D.arc_radius,
                 mallet_radius=D.mallet_radius,
                 target_radius=D.target_radius,
                 bezier_ratio=D.bezier_ratio):
        ############ Parameters ############
        self.width = width
        self.height = height
    ...
    def random_position(self, obj, top, bottom):
        return np.array([
                np.random.randint(
                        self.rink_left + obj.radius + 50,
                        self.rink_right - obj.radius - 50
                        ),
                np.random.randint(
                        top + obj.radius + 50, 
                        bottom - obj.radius - 50
                        )])